...
export default {
  ...
  methods: {
    ...
    // 把这次的请求封装成一个函数,当需要时调用即可
    getData() {
      this.$store.dispatch("getSearchList", this.searchParams);
    },
    // 删除分类的名字
    removeCategoryName(){
      // 把带给服务器的参数置空了,还需要再次向服务器发送请求
      // 带给服务器参数说明可有可无的:如果属性值为空的字符串还是会把相应的字段带给服务器
      // 但是把相应的字段设置为undefined,则这个字段不会带给服务器
      ...
      this.getData()
      // 地址栏也需要修改:进行路由跳转
      // 严谨:本意是删除query,如果路径中出现params,不应该删除,路由跳转的时候应该带着
    ...
    // 排序的操作
    changeOrder(flag){
      // flag形参：它是一个标记，代表用户点击的是综合(1)还是价格(2)[用户点击时传递过来的]
  ...
  watch:{
    ...
    $route(newValue,oldValue){
      ...
      // 再次发送请求
      this.getData()
      // 每一次请求完毕,应该把相应的1,2,3级分类的id置空,让它接收下一次的相应的1,2,3的id
